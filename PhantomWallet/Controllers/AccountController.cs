    ...
    public class AccountController
    {
            ...
            string destinationChainAddress)
        {
            try
            {
                ...
                var settleTx = new Phantasma.Blockchain.Transaction(nexusName, destinationChainName, settleTxScript, DateTime.UtcNow + TimeSpan.FromMinutes(30), "PHT-0-9-5");
            ...
            string chainName, string destinationChain, string symbol, string amountId)
        {
            try
            {
                ...
                var tx = new Phantasma.Blockchain.Transaction(nexusName, chainName, script, DateTime.UtcNow + TimeSpan.FromMinutes(30), "PHT-0-9-5");
                tx.Sign(keyPair);
            ...
            try
            {
              ...
              var script = ScriptUtils.BeginScript()
                ...
                var tx = new Phantasma.Blockchain.Transaction(nexusName, chainName, bscript,
                    DateTime.UtcNow + TimeSpan.FromMinutes(30), "PHT-0-9-5");
                tx.Sign(keyPair);
        ...
        public async Task<string> ConfirmSellNFT(PhantasmaKeys keyPair, string chainName, string contractName, string methodName, object[] paramArray, string idArr)
        {
            try
            {
              ...
              var script = ScriptUtils.BeginScript()
                    ...
                    foreach (var id in jsonparam)
                      {
                          var bigIntAmount = BigInteger.Parse((string)id["id"]);
                          paramArray[3] = bigIntAmount;
                          script.CallContract(contractName, methodName, paramArray);
                      }
        ...
        public async Task<object> CreateMultisigWallet(PhantasmaKeys keyPair, MultisigSettings settings)
        {
            try
            {
                ...
                var nexusName = WalletConfig.Network;
                var tx = new Phantasma.Blockchain.Transaction(nexusName, "main", script, DateTime.UtcNow + TimeSpan.FromMinutes(30), "PHT-0-9-5");
                ...
                PhantasmaKeys keyPair, string chain, string contract, string method, object[] paramArray)
        {
            try
            {
                ...
                var nexusName = WalletConfig.Network;
                var tx = new Phantasma.Blockchain.Transaction(nexusName, chain, script, DateTime.UtcNow + TimeSpan.FromMinutes(30), "PHT-0-9-5");
        ...
        public async Task<object> MarketBuyCustomModal(
                PhantasmaKeys keyPair, string chain, string contract, string method, object[] paramArray, string idArr, string quoteSymbol, string priceArr, string creatorArr)
        {
            try
            {

              JArray jsonparam = JsonConvert.DeserializeObject<JArray>(idArr);
              JArray creatorparam = JsonConvert.DeserializeObject<JArray>(creatorArr);
              JArray priceparam = JsonConvert.DeserializeObject<JArray>(priceArr);
              ...
              var script = ScriptUtils.BeginScript()
                ...
                var nexusName = WalletConfig.Network;
                var tx = new Phantasma.Blockchain.Transaction(nexusName, chain, bscript, DateTime.UtcNow + TimeSpan.FromMinutes(30), "PHT-0-9-5");
                ...
                PhantasmaKeys keyPair, string stakeAmount)
        {
          try
          {
            ...
            var nexusName = WalletConfig.Network;
            var stakeTx = new Phantasma.Blockchain.Transaction(nexusName, "main", script, DateTime.UtcNow + TimeSpan.FromMinutes(30), "PHT-0-9-5");
